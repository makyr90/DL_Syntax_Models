...
if __name__ == '__main__':
    ...
    if options.predictFlag:
        ...
        ts = time.time()
        test_res = list(parser.Predict(options.conll_test,True,options.pred_batch_size))
        te = time.time()
        ...
        if (options.last_epoch == 0):
            ...
            parser = learner.parser(words, pos, xpos, rels, w2i, c2i, ext_words_train, ext_words_dev, stored_opt)
            parser.Load(os.path.join(options.output, os.path.basename(options.model)))



        sentences,train_batches = utils.batch_data(options.conll_train,c2i,options.batch_tokens,True)
            ...
            for idx,mini_batch in enumerate(train_batches):
                ...
                if (t_step <= 5000):
                    ...
                    if ((t_step - tsId) > 2500):
                        ...
                        print("Model  didn't improve during the last 2500 trainning steps")
                        sys.exit()
