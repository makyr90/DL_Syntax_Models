...
class parser:
    ...
    def Predict(self,conll_sentences,test=False):
        ...
        for bdx in range(len(test_batches)):
            ...
            input_vecs2 = [dy.concatenate([f, b]) for f, b in zip(fwd_embs1, list(reversed(bwd_embs1)))]
            fwd2 = self.fwdLSTM2.initial_state(batch_size)
            bwd2 = self.bwdLSTM2.initial_state(batch_size)
            fwd_embs2 = fwd2.transduce(input_vecs2,masks,True)
    ...
    def calculate_loss(self,sentences):
        ...
        for sentence in sentences:
            ...
            heads_labels_idx.extend([0]*((len(sentences[0]) - len(heads_labels_idx))))
        ...
        return self.attention.decode_loss(src_encodings,masks, (batch_heads, batch_labels,batch_heads_labels_idx)),total_words
